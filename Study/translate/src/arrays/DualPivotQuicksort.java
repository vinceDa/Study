...
/**
 *
 * 这个类实现了Vladimir Yaroslavskiy,
 * Jon Bentley, 和Josh Bloch的Dual-Pivot快速排序算法。
 * 这个算法在大数据集的前提下有着O(n log(n))的性能表现二其他一些快速排序
 * 只能降级到二次性能, 而且它通常比传统 (one-pivot)的快速排序算法更快
 *
 * 所有暴漏的方法都是私有的, 它们在做过了所有必要的数组边界检查和必填项参数扩展后
 * 被Arrays中的公共方法调用
